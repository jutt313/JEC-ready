...
const VehicleUploadInterface: React.FC<VehicleUploadInterfaceProps> = ({ companyId }) => {
  ...
  const generateDoc1 = async (fontBytes: ArrayBuffer, exportData: ExportData, companyData: CompanyData) => {
    ...
    form.getTextField('cumpany name in katakana').setText(companyData.name_katakana || '');
    ...
    form.getTextField('company phone number or personal').setText(companyData.phone || '');
  ...
  const processFile = async (uploadedFile: UploadedFile) => {
    ...
    try {
      ...
      console.log(`OCR completed, extracted text length: ${ocrResponse.data.extractedText?.length || 0}`);
