...
export const CreateCompanyModal: React.FC<CreateCompanyModalProps> = ({
  ...
  const validateKatakana = (text: string): boolean => {
    // Allow Katakana, digits, spaces
    const katakanaRegex = /^[\u30A0-\u30FF\u30FC-\u30FE0-9\s]*$/;
    return katakanaRegex.test(text);
  };

  const validateKanjiHiragana = (text: string): boolean => {
    // Allow Kanji, Hiragana, Katakana, digits, spaces
    const kanjiHiraganaRegex = /^[\u3040-\u309F\u4E00-\u9FAF\u3400-\u4DBF\u30A0-\u30FF\u30FC-\u30FE0-9\s]*$/;
    return kanjiHiraganaRegex.test(text);
  };

  const validateJusoCode = (jusoCode: string): boolean => {
    const jusoRegex = /^\d{5}$/;
    return jusoRegex.test(jusoCode);
  };
  ...
  const handleInputChange = (field: keyof typeof formData, value: string) => {
    ...
    if (field === 'nameKatakana' && value && !validateKatakana(value)) {
      newErrors[field] = 'カタカナのみ入力してください / Please enter only Katakana characters';
    }
    
    if (field === 'nameKanji' && value && !validateKanjiHiragana(value)) {
      newErrors[field] = '漢字・ひらがなのみ入力してください / Please enter only Kanji/Hiragana characters';
    }
    
    if (field === 'addressJapanese' && value && !validateKanjiHiragana(value)) {
      newErrors[field] = '漢字・ひらがなのみ入力してください / Please enter only Kanji/Hiragana characters';
    }
    
    if (field === 'jusoCode' && value && !validateJusoCode(value)) {
      newErrors[field] = '正しい住所コードを入力してください / Please enter a valid JUSO code (5 digits)';
    }
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!validateKanjiHiragana(formData.nameKanji)) {
      newErrors.nameKanji = '漢字・ひらがなのみ入力してください / Please enter only Kanji/Hiragana characters';
    }

    if (!formData.nameKatakana.trim()) {
      newErrors.nameKatakana = 'カタカナの会社名は必須です / Katakana company name is required';
    } else if (!validateKatakana(formData.nameKatakana)) {
      newErrors.nameKatakana = 'カタカナのみ入力してください / Please enter only Katakana characters';
    }

    if (!formData.addressJapanese.trim()) {
      newErrors.addressJapanese = '会社住所は必須です / Company address is required';
    } else if (!validateKanjiHiragana(formData.addressJapanese)) {
      newErrors.addressJapanese = '漢字・ひらがなのみ入力してください / Please enter only Kanji/Hiragana characters';
    }
    ...
    } else if (!validateJusoCode(formData.jusoCode)) {
      newErrors.jusoCode = '正しい住所コードを入力してください / Please enter a valid JUSO code (5 digits)';
    }
